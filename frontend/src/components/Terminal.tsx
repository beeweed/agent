...
export function TerminalPanel({ className = "" }: TerminalProps) {
  ...
  const rendererRef = useRef<"webgl" | "canvas" | "dom">("dom");
  ...
  const connectWs = useCallback(() => {
    ...
    ws.onopen = () => {
      ...
      // Send current terminal dimensions right away
  ...
  useEffect(() => {
    ...
    // Try GPU-accelerated rendering, fall back gracefully
    try {
    ...
    // Handle user keyboard input → send to backend
    term.onData((data) => {
      sendInput(data);
    });

    // Handle binary data (mouse events, special sequences from TUI apps)
    term.onBinary((data) => {
      sendBinary(data);
    });

    // When terminal dimensions change → notify backend to resize PTY
    ...
    // Resize observer for auto-fit on container size change
  ...
  }, [sandboxStatus, connectWs, setTerminalConnected, clearPingInterval]);
  ...
  }, [connectWs, setTerminalConnected, clearPingInterval]);
  ...
  }, [searchVisible, handleSearchOpen, handleSearchClose]);

  // Maximize / minimize
  const { terminalHeight, setTerminalHeight } = useStore();
  ...
  // Re-fit when terminal height changes (from resize handle)
  useEffect(() => {
    debouncedFit();
  }, [terminalHeight, debouncedFit]);
  ...
  return (
    ...
    >
      ...
      >
        <div data-design-id="terminal-header-left" className="flex items-center gap-2">
          ...
          {rendererRef.current !== "dom" && (
            <span
              data-design-id="terminal-renderer-badge"
              className="text-[9px] text-[#565f89] font-mono bg-[#292e42] px-1.5 py-0.5 rounded"
            >
              {rendererRef.current === "webgl" ? "GPU" : "Canvas"}
            </span>
          )}
        </div>

        <div data-design-id="terminal-header-right" className="flex items-center gap-1">
          {/* Search button */}
          <button
            data-design-id="terminal-search-btn"
            onClick={searchVisible ? handleSearchClose : handleSearchOpen}
          ...
          <button
            ...
            className="p-1 rounded hover:bg-[#292e42] text-[#565f89] hover:text-[#a9b1d6] transition-colors"
          ...
          {/* Connection status */}
          <div data-design-id="terminal-status-group" className="flex items-center gap-1 ml-1">
            {terminalConnected ? (
      ...
      {searchVisible && (
        ...
        >
          ...
          <input
            data-design-id="terminal-search-input"
            type="text"
            value={searchQuery}
            onChange={(e) => {
          ...
          />
          <button
            data-design-id="terminal-search-prev-btn"
          ...
          <button
            data-design-id="terminal-search-next-btn"
          ...
          <button
            data-design-id="terminal-search-close-btn"
      ...
      <div
        ...
        onContextMenu={(e) => {
          // Right-click paste (like VS Code terminal)
          e.preventDefault();
          navigator.clipboard
            .readText()
            .then((text) => {
