...
import { Code, Eye, Check, Loader2, AlertCircle, BookOpen, Replace, Plus, Trash2, Eraser, TerminalSquare, ChevronDown, ChevronRight, MonitorUp } from "lucide-react";
...
export function ChatMessage({ entry }: ChatMessageProps) {
  ...
  if (entry.type === "bash_view_card") {
    return <BashViewToolBlock entry={entry} />;
  }
...
function BashViewToolBlock({ entry }: { entry: ChatEntry }) {
  const [outputExpanded, setOutputExpanded] = useState(false);

  const isViewing = entry.bashViewStatus === "viewing";
  const isViewed = entry.bashViewStatus === "viewed";
  const isError = entry.bashViewStatus === "error";
  const result = entry.bashViewResult;
  const output = result?.output || "";
  const sessionStatus = result?.status || "";
  const errorMsg = result?.error || "";

  return (
    <div data-design-id={`bash-view-card-${entry.id}`} className="animate-fade-in pl-6 xs:pl-8 sm:pl-10">
      <div
        className={`rounded-lg border overflow-hidden transition-all ${
          isViewing
            ? "border-[#bb9af7]/40 bg-[#bb9af7]/5"
            : isError
              ? "border-[#f7768e]/40 bg-[#f7768e]/5"
              : "border-[#7dcfff]/40 bg-[#7dcfff]/5"
        }`}
      >
        {/* Header */}
        <div
          data-design-id={`bash-view-card-header-${entry.id}`}
          className="flex items-center gap-2 px-3 py-2 bg-[#1a1b26]/60"
        >
          <div className="flex items-center gap-1.5 flex-shrink-0">
            {isViewing && (
              <Loader2 className="w-3.5 h-3.5 text-[#bb9af7] animate-spin" />
            )}
            {isViewed && (
              <Check className="w-3.5 h-3.5 text-[#7dcfff]" />
            )}
            {isError && (
              <AlertCircle className="w-3.5 h-3.5 text-[#f7768e]" />
            )}
            <MonitorUp className="w-3.5 h-3.5 text-[#bb9af7]" />
          </div>

          <span
            data-design-id={`bash-view-card-display-name-${entry.id}`}
            className="text-xs font-medium text-[#a9b1d6] truncate flex-1"
          >
            View bash session output
          </span>

          {entry.bashViewSessionName && (
            <span
              data-design-id={`bash-view-card-session-${entry.id}`}
              className="text-[9px] font-mono text-[#565f89] bg-[#292e42] px-1.5 py-0.5 rounded flex-shrink-0"
            >
              {entry.bashViewSessionName}
            </span>
          ...
          {isViewed && sessionStatus && (
            <span
              data-design-id={`bash-view-card-status-${entry.id}`}
              className={`text-[9px] font-mono px-1.5 py-0.5 rounded flex-shrink-0 ${
        ...
        {/* Output dropdown — default collapsed */}
        {(isViewed || isError) && (output || errorMsg) && (
          <div data-design-id={`bash-view-card-output-section-${entry.id}`} className="border-t border-[#292e42]/60">
            <button
              data-design-id={`bash-view-card-output-toggle-${entry.id}`}
              onClick={() => setOutputExpanded(!outputExpanded)}
              className="flex items-center gap-1.5 w-full px-3 py-1.5 text-left hover:bg-[#1a1b26]/40 transition-colors"
            >
              {outputExpanded ? (
            ...
            {outputExpanded && (
              <div
                data-design-id={`bash-view-card-output-content-${entry.id}`}
                className="px-3 pb-2"
              >
                <div className="bg-[#1a1b26] rounded-md p-2 max-h-[300px] overflow-auto">
                  <pre className="text-[11px] font-mono text-[#a9b1d6] whitespace-pre-wrap break-all">
